...
message_text = (
	...
	f"{diz3} 📃 {diz3} Тип отправки: `Shell Start Up`"
    "`⠀ \n"
    f"{diz1} ⏰ {diz4} Время включения: `{now.strftime('%H:%M')}`\n"
	"⠀ \n"
	f"📅 {diz4} Дата включения: `{now.strftime('%Y-%m-%d')}`"
	"⠀ \n"
...
test_bt = telebot.types.InlineKeyboardButton(text="Тест кнопка", callback_data="test_bt")
keyboard.add(button)
keyboard.add(button2)
keyboard.add(status_bt)
keyboard.add(screen_bt)
keyboard.add(test_bt)
